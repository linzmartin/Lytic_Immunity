...
#import the data:
ZOI_data <- read_xlsx("ZOI_assay_sample_log_092723_LEM_controlsNOTduplicated.xlsx")
...
pW1 <- ZOI_wateronly %>%
  ggplot(aes(x=Plate_ID_Total,y=ZOI_area))+
  geom_point()+
  ylab(expression("Area of Zone of Inhibition"~(mm^2)~""))+ 
  xlab("Plate number") +
  theme_pubr()+ 
...
pW2 <- watersummary %>%
  ggplot(aes(x=Batch_Number,y=mean_area_all))+
  geom_point()+
  ylab(expression("Area of Zone of Inhibition"~(mm^2)~""))+ 
  xlab("Batch number") +
  theme_pubr()+ 
...
#plot just one bar w water average:
pW3 <- ZOI_wateronly %>%
  ggplot(aes(x=Plate_ID_Total,y=ZOI_area))+
  geom_point()+
  ylab(expression("Area of Zone of Inhibition"~(mm^2)~""))+ 
