...
#mean diameter with jitter for points:
png(filename = "ZOI_diameter_jitter_081123.png", width = 5, height = 4, units = "in", res = 300)
...
#mean area with jitter for points:
png(filename = "ZOI_area_jitter_081123.png", width = 5, height = 4, units = "in", res = 300)
ZOI_summary_bioreps_italic %>%
  group_by(Treatment)%>%
  ggplot(aes(x=Age,y=mean_area_all))+
  geom_bar(aes(color = Treatment, 
...
png(filename = "ZOI_diameter_jitter_081123_ageattop.png", width = 5, height = 4, units = "in", res = 300)
ZOI_summary_bioreps_italic %>%
  group_by(Treatment)%>%
  ...
  geom_errorbar(aes(ymin=mean_diameter_all - SE_diameter_all,
                ...
                color="black")+
  #scale_x_discrete(limits = c("Naïve","LB","E_coli","M_luteus"),labels=c("Naïve","LB","E. coli","M. luteus"))+
  ylab(expression("Mean diameter of ZOI"~(mm)~""))+ xlab("Temperature (˚C)") +
  theme_pubr()+
  #theme_bw()+
  #theme(text = element_text(size = 26))+
  #theme(axis.text.x = element_text(angle=45, hjust=1))+
...
#mean area with jitter for points:
png(filename = "ZOI_area_jitter_081123_ageattop.png", width = 5, height = 4, units = "in", res = 300)
ZOI_summary_bioreps_italic %>%
  group_by(Treatment)%>%
