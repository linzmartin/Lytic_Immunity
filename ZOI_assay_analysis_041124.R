...
full_anova #F for treatment: 42.635811
...
eta_squared(anova(lm_4,type=2,ddf ="Kenward-Roger"),parital=TRUE)


fullmodel_effectsize_partial_dataframe <- as.data.frame(eta_squared(anova(lm_4,type=2,ddf ="Kenward-Roger"),parital=TRUE))
...
#emmeans(lm_4, pairwise~Temperature_centered|Age_centered,
 #       type="response",
     #   at = list(Temperature_centered = c(-1.2,1.19,0.23),
      #            Age_centered = c(-1.25,-0.47,0.5,1.48)))
...
full_TREATMENTonly <- full_Ecemm_TREATMENTonly %>%
  ...
  theme(text = element_text(size=12),
        ...
        axis.text.y=element_text(size=10))+
  theme(legend.position = "none")#+
  #geom_signif(data=full_Ecemm_TREATMENTonly,stat="signif",position="identity",
   #           comparisons=list(c("Naïve","Injury",bquote(italic("E. coli")), bquote(italic("M. luteus")))),map_signif_level = TRUE)
full_TREATMENTonly
...
#treat_comparison<-pairs(full_emm,~c("Treatment"),adjust="Tukey")
#treat_comparison <- as.data.frame(treat_comparison)
###
#pigs.emm.s <- emmeans(lm_4, "Treatment",type="response")
#pairs(pigs.emm.s)
#head(pigs.emm.s)
...
library(multcomp)
summary(lm_4)
summary(glht(lm_4, linfct = mcp(Treatment = "Tukey")))

treatment_fullmodel_multcomp <- summary(glht(lm_4, linfct = mcp(Treatment = "Tukey")))
treatment_fullmodel_multcomp

#save outputs:
sink("treatment_fullmodel_multcomp.txt")
print(treatment_fullmodel_multcomp)
sink()


#pairwise.t.test(ZOI_data$ZOI_area, ZOI_data$Treatment,
 #               p.adjust.method = "BH")
...
###############################
#correlation btwn plate dose and zoi:

# Shapiro-Wilk normality test for Infectious_Dose_for_Plates
shapiro.test(ZOI_data$Infectious_Dose_for_Plates) # => p < 0.001
# Shapiro-Wilk normality test for ZOI_area
shapiro.test(ZOI_data$ZOI_area) # => p < 0.001
#non-normal

sink("seededplates_cor_normality_test.txt")
shapiro.test(ZOI_data$Infectious_Dose_for_Plates) # => p < 0.001
# Shapiro-Wilk normality test for ZOI_area
shapiro.test(ZOI_data$ZOI_area)
sink()

cor(x=ZOI_data$Infectious_Dose_for_Plates,y=ZOI_data$ZOI_area,method="spearman",use="complete.obs")
seeded_cortest <-cor.test(ZOI_data$Infectious_Dose_for_Plates,ZOI_data$ZOI_area,method="spearman",use="complete.obs")
seeded_cortest

Seeded_plates_correlation <- ZOI_data %>%
  ggscatter(x="Infectious_Dose_for_Plates",y="ZOI_area",
            add="reg.line",conf.int = TRUE,cor.coef = TRUE,
            cor.method = "spearman",
            cor.coef.coord = c(2e8, 100))+
  #facet_grid(~Temperature)+
  ylab(expression("Area of Zone of Inhibition"~(mm^2)~""))+ 
  xlab(expression(paste("Dose of ",italic("M. luteus"), " bacteria seeded per plate")))+
  theme_pubr()+ 
  theme(panel.background = element_rect(fill = NA, color = "black"))+
  theme(panel.spacing = unit(0.6, "lines"))+
  ...
  theme(text = element_text(size=12),
        ...
        axis.text.y=element_text(size=10))+
  theme(legend.position = "none")
Seeded_plates_correlation

ggsave("Seeded_plates_correlation.png",plot=Seeded_plates_correlation,width = 5, height = 5, units = "in", dpi = 600)
#ggsave("Seeded_plates_correlation.eps",plot=Seeded_plates_correlation,width = 5, height = 4, units = "in", dpi = 600)
ggsave("Seeded_plates_correlation.pdf",plot=Seeded_plates_correlation,width = 5, height = 5, units = "in", dpi = 600)

sink("seeded_correlation_test.txt")
print(seeded_cortest)
sink()  

#correlation btwn injected dose and zoi:
#ecoli:
# Shapiro-Wilk normality test for Infectious_Dose_for_Plates
shapiro.test(Ecoli_ZOI$Injected_CFU_calculated_dose) # => p = 0.003
# Shapiro-Wilk normality test for ZOI_area
shapiro.test(Ecoli_ZOI$ZOI_area) # => p < 0.001
#non-normal

sink("Ecoli_cor_normality_test.txt")
# Shapiro-Wilk normality test for Infectious_Dose_for_Plates
shapiro.test(Ecoli_ZOI$Injected_CFU_calculated_dose) # => p = 0.003
# Shapiro-Wilk normality test for ZOI_area
shapiro.test(Ecoli_ZOI$ZOI_area) # => p < 0.001
...
cor(x=Ecoli_ZOI$Injected_CFU_calculated_dose,y=Ecoli_ZOI$ZOI_area,method="spearman",use="complete.obs")

Ecoli_cortest <-cor.test(Ecoli_ZOI$Injected_CFU_calculated_dose,Ecoli_ZOI$ZOI_area,method="spearman",use="complete.obs")
Ecoli_cortest

Ecoli_correlation <- Ecoli_ZOI %>%
  ggscatter(x="Injected_CFU_calculated_dose",y="ZOI_area",
            add="reg.line",conf.int = TRUE,cor.coef = TRUE,
            cor.method = "spearman",
            cor.coef.coord = c(15000, 100))+
  #facet_grid(~Temperature)+
  ylab(expression("Area of Zone of Inhibition"~(mm^2)~""))+ 
  xlab(expression(paste("Infectious dose of ",italic("E. coli"), " per mosquito")))+
  theme_pubr()+ 
  theme(panel.background = element_rect(fill = NA, color = "black"))+
  theme(panel.spacing = unit(0.6, "lines"))+
  scale_y_continuous(limits=c(0,100))+
...
# Shapiro-Wilk normality test for Infectious_Dose_for_Plates
shapiro.test(Mlut_ZOI$Injected_CFU_calculated_dose) # => p < 0.001
# Shapiro-Wilk normality test for ZOI_area
shapiro.test(Mlut_ZOI$ZOI_area) # => p < 0.001
#non-normal

sink("Mlut_cor_normality_test.txt")
# Shapiro-Wilk normality test for Infectious_Dose_for_Plates
shapiro.test(Mlut_ZOI$Injected_CFU_calculated_dose) # => p < 0.001
# Shapiro-Wilk normality test for ZOI_area
shapiro.test(Mlut_ZOI$ZOI_area) # => p < 0.001
...
cor(x=Mlut_ZOI$Injected_CFU_calculated_dose,y=Mlut_ZOI$ZOI_area,method="spearman",use="complete.obs")

Mlut_cortest <-cor.test(Mlut_ZOI$Injected_CFU_calculated_dose,Mlut_ZOI$ZOI_area,method="spearman",use="complete.obs")
Mlut_cortest


Mlut_correlation <- Mlut_ZOI %>%
  ggscatter(x="Injected_CFU_calculated_dose",y="ZOI_area",
            add="reg.line",conf.int = TRUE,cor.coef = TRUE,
            cor.method = "spearman",
            cor.coef.coord = c(10000, 100))+
  #facet_grid(~Temperature)+
  ylab(expression("Area of Zone of Inhibition"~(mm^2)~""))+ 
  xlab(expression(paste("Infectious dose of ",italic("M. luteus"), " per mosquito")))+
  theme_pubr()+ 
  theme(panel.background = element_rect(fill = NA, color = "black"))+
  theme(panel.spacing = unit(0.6, "lines"))+
  scale_y_continuous(limits=c(0,100))+
...
ggsave("Ecoli_correlation.png",plot=Ecoli_correlation,width = 5, height = 5, units = "in", dpi = 600)
ggsave("Ecoli_correlation.pdf",plot=Ecoli_correlation,width = 5, height = 5, units = "in", dpi = 600)
ggsave("Mlut_correlation.png",plot=Mlut_correlation,width = 5, height = 5, units = "in", dpi = 600)
ggsave("Mlut_correlation.pdf",plot=Mlut_correlation,width = 5, height = 5, units = "in", dpi = 600)


sink("Ecoli_correlation_test.txt")
print(Ecoli_cortest)
sink() 

sink("Mlut_correlation_test.txt")
print(Mlut_cortest)
sink() 
...
Naive_Ageonly <- Naive_Ecemm_AGEonly %>%
  ...
  labs(x="Adult Age (days)")+ 
  theme_pubr()+  
  scale_fill_manual(values=rev(c("#DCD1E9","#BAA4D3","#9776BE","#7549A8")))+
  theme(panel.background = element_rect(fill = NA, color = "black"))+
  theme(panel.spacing = unit(0.6, "lines"))+
  scale_y_continuous(limits=c(0,40))+
  theme(text = element_text(size=12),
